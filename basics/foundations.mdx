...
Una firma _autógrafa_ tradicional es un signo distintivo que una persona pone en un documento para manifestar su voluntad de aceptar las condiciones que en él se establecen. En el mundo digital, una firma es un conjunto de datos que se adjunta a un documento para garantizar su autenticidad y la identidad de su autor.

En el caso concreto de las firmas electrónicas basadas en criptografía como las que produce Plumaa ID, los datos de la firma están lógicamente relacionados a un identificador, mismo que está vinculado al firmante a través de un certificado digital.

Contrario a lo que se podría pensar, una firma electrónica no es una imagen de la firma autógrafa del firmante, sino un conjunto de datos que se generan a partir de la información del documento y la <Tooltip tip="Es un número secreto muy largo usualmente entregado en un archivo (.key), y que a través de fórmulas matemáticas produce un resultado único para el firmante y el documento">clave privada</Tooltip> del firmante.

## La expresión de la voluntad

En el mundo físico, la expresión de la voluntad normalmente se manifiesta a través de la escritura de un nombre o la realización de un dibujo. Por tanto, su prueba en juicio se basa en la comparación de la firma con la de un documento de identidad o con una firma auténtica.
...
El primero usualmente toma la forma de una _firma autógrafa en dispositivo digital_ o a través del _intercambio de claves_ mayormente basados en esquemas simples como el NIP de una tarjeta de crédito.

Por el contrario, el segundo enfoque proporciona la capacidad de garantizar la autenticidad de la firma y la identidad del firmante a través de métodos matemáticos, lo que remueve la subjetividad. La tecnología de Plumaa ID depende mayormente de servicios abiertos debido a que su seguridad es garantizada a través de la transparencia y la descentralización.
...
En México, el [Código de Comercio](https://www.diputados.gob.mx/LeyesBiblio/pdf/CCom.pdf) considerará para actos de comercio se a una firma como _firma electrónica avanzada_ si cumple con los siguientes requisitos en conformidad con su artículo 97:
...
1. El firmante es identificable
2. Nadie más que el firmante tiene acceso a la llave privada que firma
3. La firma electrónica es íntegra
4. La información firmada es íntegra
...
La aplicación móvil de Plumaa ID garantiza que el firmante tiene el control total de su llave privada. Esto es posible a través de técnicas de cifrado criptográfico que utilizan tecnologías implementadas en dispositivos móviles._createMdxContent

Con estas tecnologías, la llave privada nunca abandona el dispositivo del firmante, lo que garantiza que nadie más que el firmante pueda firmar documentos en su nombre. Nisiquiera para el equipo de Plumaa ID es posible acceder a la llave privada del firmante.
...
El algoritmo de firma es el método matemático que se utiliza para generar la firma electrónica. El SAT y otros PSCs utiliza el algoritmo [RSA](https://es.wikipedia.org/wiki/RSA) con una longitud de llave de 2048 bits.
...
## Infraestructura de Seguridad Extendida (IES)
...
En el caso de que un usuario pierda su llave privada y el acceso total a su firma, este puede revocar su certificado a través del PSC emisor. De esta forma, Plumaa ID es incapaz de procesar una firma que provenga de un certificado previamente revocado._createMdxContent
